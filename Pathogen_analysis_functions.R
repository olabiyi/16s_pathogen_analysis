...
#function to perform pathogen analysis
pathogen_analysis <- function(treatment, pathogen_file){
  #read in the pathogen file for the specific treatment. This is the output from the pathogen
  #search in unix using qiime and bash commands
  #if check if the file is empty
  if (file.size(pathogen_file) > 0){
    ...
    #create dataframes for each taxon level by pasting together the appropriate columns
    ...
    #combine the vectors in one dataframe
  ...
  #put them all in one list to be returned by the function
...
draw_pathogen_plots <- function(Data,main, x, y,Xlab, YLab, plotType = 'box', group) {
  ...
  #Data is a dataframe for plotting - note that the data for the box plots need to be stacked
  ...
  if (plotType == 'bar'){
    ...
    plot.gg <- ggboxplot(data = Data, 
                         ...
                         ylab = YLab,
                         title = main)
    #remove tick lebels so the plots will look less congested
...
make_OTU_table <- function(taxon_level, samples,samples_column,
  ...
  #samples are is a character vector of the sample names i.e the samples you got it's results from blast
  #sample columns is a string that specifies the name of the column in all_frequency table
  #that contains the sample names usually will be 'Treatment'
  #frequency_column is a string that specifies thye column in all_frequency_tables
  ...
  #remove the the sample in which no pathogens were detected from the frequency table
  ...
  #loop over every sample and with each sample loop over every pathogen
...
normalize_otu_table <- function(OTU_table, method='rarefy') {
  #this fuction performs normalization on a given count otu table 'OTU_Table
  #don't forget to load the libraries and the functions
  #method can either be c('proportion', 'RLE', 'logUQ','TMM', 'DESeqVS', 'CSS', 'rarefy')
  #it performs normalization by rarefying by default  
  
  
  #change OTU table into a phyloseq object or MRexperiment obj for metagenoneSeq 
  ...
  if (method == 'proportion'){
    ...
    print("incorrect option choosen: option are 'proportion', 'RLE', 'logUQ','TMM', 'DESeqVS', 'CSS', 'rarefy' ")
...
make_taxon_tables <- function(samples,taxons,all_frequency_tables, method = 'logUQ') {
  #this function makes a taxon tables from frequency tables supplied which is a output
  #of the combine frequency table function. It output raw taxon and normalized taxon tables
  ...
  #method is a string that specifies the normalization method any of 
...
generate_stats_table <- function(taxon_table, mapping_file, independent_variables){
  ...
  map <- map[common.ids,]
  #get relative abundance normalization per sample
...
ordination_plots <- function(taxon_table, mapping_file, independent_variables, label) {
  ...
  colnames(pc.bray) <- c('PC1', 'PC2') #name the columns
  ...
  # pco.var <- apply(X = pc.bray, MARGIN = 2, FUN = sd)
  # ### calculate the percentage of variation that each PC accounts for...
  # pco.var.per <- round(pco.var/sum(pco.var)*100, 1)
  
  ####ggtext(data = pca.stat.df, x = 'PC1', y = 'PC2', labels = 'samples')
  
  #get the independent variables
  independent_variables.df <- subset(x = map, select = independent_variables)
  
  #create a PCOA stat dataframe
  pca.stat.df <- data.frame(independent_variables.df,samples = rownames(pc.bray), PC1 = pc.bray[,'PC1'], PC2 = pc.bray[,'PC2'])
  ...
  for (variable in independent_variables) {
    p<- ggscatter(data = pca.stat.df, x = 'PC1', y = 'PC2', 
    ...
    print(ggscatter(data = wa_stat.df, x = 'wa1', y = 'wa2',
                    label = 'taxon', ggp = p))
    # + 
    #   geom_exec(geomfunc = geom_point, data = wa_stat.df, x = 'wa1', y = 'wa2',size = 'wa1')
...
generate_abundance_tables <- function(independent_variables,variable,taxon_table) {
  #function to generate abundance table and frequency table of a specied variable
  #from a taxon table
  #-independent_variables is a dataframe of independent variables or a factor vector of
  #an indenpendent variable
  #-variable is a string specifying the name of the independent variable for whom you
  #would like to derive it's frequency and avundance tables
  ...
  #create a list of list which is the same lenght as the levels of independent variable
